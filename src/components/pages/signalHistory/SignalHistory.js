...
function InteractiveGraph() {
  ...
  const handleMouseMove = (e) => {
    ...
    // Calculate a mock price based on position
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col justify-between p-4 py-8 pointer-events-none opacity-20">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className="w-full border-t border-dashed border-white/40"
          />
        ))}
      </div>
      ...
      >
        ...
        {/* Area Fill */}
        <path
          d="M 0 100 L 0 80 L 50 60 L 100 90 L 150 40 L 200 70 L 250 30 L 300 45 L 350 10 L 400 35 L 450 25 L 500 50 L 500 100 Z"
        ...
        {/* Main Price Line */}
        <path
          d="M 0 80 L 50 60 L 100 90 L 150 40 L 200 70 L 250 30 L 300 45 L 350 10 L 400 35 L 450 25 L 500 50"
          fill="none"
          stroke="#F3BE68"
          strokeWidth="1.5"
          strokeLinecap="round"
          className="drop-shadow-[0_0_8px_rgba(243,190,104,0.5)]"
        />
        ...
        <circle
          ...
          r="3"
          fill="#02C16D"
          className="animate-pulse shadow-[0_0_10px_#02C16D]"
        />
        <circle
          cx="350"
          cy="10"
      ...
      {hoverData.visible && (
        <>
          <div
            className="absolute top-0 bottom-0 w-[1px] bg-white/20 pointer-events-none transition-transform duration-75"
            style={{ transform: `translateX(${hoverData.x}px)` }}
          />
          <div
            className="absolute z-30 pointer-events-none transition-all duration-150 ease-out flex flex-col items-center"
            style={{ left: hoverData.x, top: "20%" }}
          >
            <div className="bg-black/90 border border-[#F3BE68]/50 p-3 rounded-xl backdrop-blur-xl shadow-2xl -translate-x-1/2">
              <p className="text-[8px] text-[#F3BE68] uppercase font-black tracking-tighter">
                Live Tracker
              </p>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <div className="w-1.5 h-1.5 rounded-full bg-[#02C16D]" />
                <span className="text-[9px] text-[#D3C4B2]">
                  98.4% Precision
                </span>
              </div>
            </div>
...
function MetricCard({ label, value, color }) {
  return (
    <div className="group bg-[#1C1B1B] py-2 px-4 rounded-2xl border border-white/10 flex-1 md:min-w-[140px] items-center hover:border-white/30 transition-all duration-300">
      ...
      <p
        className="text-2xl font-black mt-1 transition-transform group-hover:scale-105"
        style={{ color }}
      >
        {value}
      </p>
...
function TableHeader() {
  return (
    <div className="grid grid-cols-6 text-[9px] uppercase font-bold tracking-[2px] text-white/30 border-b border-white/5 p-6 bg-white/[0.01]">
      <div>Date</div>
      <div>Status</div>
      <div>Pair</div>
      <div>Strategy</div>
...
function TableRow({ status, change, color, delay }) {
  return (
    <div
      className="grid grid-cols-6 p-6 border-b border-white/[0.03] items-center hover:bg-white/[0.04] transition-all duration-300 group cursor-pointer animate-fade-in-up"
    ...
    >
      ...
      <div className="text-xs font-bold text-[#E5E2E1] group-hover:text-[#F3BE68] transition-colors">
        USD / NGN
      </div>
      <div className="text-xs text-[#D3C4B2]/70">Spread Break</div>
      ...
      <div className="text-right font-bold font-mono text-sm" style={{ color }}>
        {change}
      </div>
    </div>
  );
